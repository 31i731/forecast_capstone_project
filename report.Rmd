...
#sarima_forecast <- sarima.for(training, n.ahead = length(testing), p=0,d=1,q=1, P=1, D=1, Q=0)
```

```{r}
arima_basic <- auto.arima(training, max.D = 5)
arima_basic

plot(forecast(arima_basic, h=365.25/7))
...
Forecast with cloud cover:

```{r}
arima_cloud <- auto.arima(training, xreg=matrix(weather[1:length(training),cloudcover]))
arima_optimal

plot(forecast(arima_cloud, h=365.25/7, xreg=matrix(weather[109:length(weather),cloudcover])))
lines(testing, col="red")
```

```{r}
#length(training)
matrix(weather[1:108, uvIndex], weather[1:108, cloudcover])
```

Forecast with UV index:

```{r}
arima_uv <- auto.arima(
                    training, 
                    xreg = as.matrix(weather[1:108, .(uvIndex, cloudcover)])
                    )
arima_uv
```
...
plot(forecast(
  arima_uv, 
  h=52, 
  xreg = as.matrix(weather[109:length(weather), .(uvIndex, cloudcover)])
  ))
#lines(testing, col="red")
...
```{r}
fc <- forecast(arima_uv, h=365.25/7, xreg=matrix(weather[109:length(weather),uvIndex]))
fc$mean
length(fc$mean)
```

```{r}
#error estimation& accumulation
plot(x=1:length(fc$mean), cumsum(abs(fc$mean)),type="l", col="red")
# lines(x=1:length(testing), cumsum(abs(arima_cloud$fitted - testing)),type="l", 
# col="green")
# lines(x=1:length(testing), cumsum(abs(arima_uv$fitted - testing)),type="l", 
# col="brown")
lines(x=1:length(testing), cumsum(abs(sarima_forecast$pred)),type="l", col="orange")
```
